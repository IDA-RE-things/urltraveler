...
class SubLayoutUI : public CContainerUI
{
public:
	SubLayoutUI(LPTSTR lpszLayoutName)
	{
		CDialogBuilder builder;
		CContainerUI* pLayoutUI = static_cast<CContainerUI*>(builder.Create(lpszLayoutName, (UINT)0));
		if( pLayoutUI ) {
			this->Add(pLayoutUI);
		}
		else {
...
public:
	...
	{
		wchar_t szLayoutName[MAX_PATH] = {0};

		_stprintf_s(szLayoutName, MAX_PATH - 1, _T("%s.xml"), pstrClass);

		BOOL bRet = PathFileExists(szLayoutName);

		return new SubLayoutUI(szLayoutName);
	}
};
...
void CLayoutManager::Init(HWND hWnd,LPCTSTR pstrLoad)
{
	...
	if(*pstrLoad!='\0')
	{
		...
		CControlUI* pRoot=builder.Create(pstrLoad,(UINT)0,&cb,&m_Manager);
		if(pRoot)
			pForm->Add(pRoot);
...
CControlUI* CLayoutManager::NewUI(int nClass,CRect& rect,CControlUI* pParent)
{
	...
	case classListHeader:
		...
		pExtended->nClass=classListHeader;
		pExtended->nDepth = 0;
		break;
	case classListHeaderItem:
		pControl = new CListHeaderItemUI;
		pExtended->nClass=classListHeaderItem;
		pExtended->nDepth=0;
		break;
	case classListBody:
		pControl=new CListBodyUI((CListUI *)pParent);
		pExtended->nClass=classListBody;
		pExtended->nDepth=0;
		break;
	case classControl:
	...
	if (nClass == classList)
	{
		...
		tmpRect.bottom = rect.top + 30;
		NewUI(classListHeader, tmpRect, pControl);

		tmpRect.left = rect.left;
		tmpRect.top = rect.top + 30;
		tmpRect.right = rect.right;
		tmpRect.bottom = rect.bottom;
		NewUI(classListBody, tmpRect, pControl);
	}
...
CControlUI* CLayoutManager::CopyControl(CControlUI* pControl)
{
	...
	switch( cchLen ) {
		case 6:
			...
			else if( _tcscmp(pstrClass, _T("ListUI")) == 0 )              pCopyControl = new CListUI(*static_cast<CListUI*>(pControl->GetInterface(_T("CList"))));
		...
		case 10:
			...
			else if( _tcscmp(pstrClass, _T("ListBodyUI")) == 0 )          pCopyControl = new DuiLib::CListBodyUI(*static_cast<DuiLib::CListBodyUI*>(pControl->GetInterface(_T("ListBody"))));
			break;
		case 11:
...
// 		SaveTileLayoutProperty(pControl, pNode);
		//break;
	case classList:
		//SaveListHeaderItemProperty(((CListUI *)pControl)-(), pNode);
...
void CLayoutManager::SaveSkinFile(LPCTSTR lpszPathName)
{
	...
	{
		_stprintf_s(szBuf, _T("%d,%d"), szMaxWindow.cx, szMaxWindow.cy);
...
void CLayoutManager::SetDefaultUIName(CControlUI* pControl, BOOL bForce/* = FALSE*/)
{
	...
	{
	case classListHeader:
		pCount = &m_UINameCount.nListHeaderCount;
		break;
	case classListHeaderItem:
		pCount = &m_UINameCount.nListHeaderItemCount;
		break;
	case classListBody:
		pCount = &m_UINameCount.nListBodyCount;
		break;
	case classList:
		pCount = &m_UINameCount.nListCount;
		break;
	case classControl:
