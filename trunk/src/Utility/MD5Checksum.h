...
class UTILITY_API CMD5Checksum  
{
public:
	/**
	 * Function		对给定的字符串缓冲区进行MD5计算
	 * @param		pBuf 需要计算的缓冲区的长度
	 * @param		nLength	缓冲区的长度
	 * @return		返回该缓冲区对应的MD5值
	 **/
	static string GetMD5( BYTE* pBuf, UINT nLength);

	/**
	 * Function		对给定的文件进行MD5计算
	 * @param		strFilePath MD5文件所在的路径
	 * @return		对文件中的内容进行MD5计算的结果
	 **/
	static string GetMD5( const string& strFilePath);
	//  [2/20/2009 ken]
	static string GetMD5W( const wstring& strFilePath);

	/**
	 * 这个函数主要用于对文件中的一部分进行MD5算法
	 * @param		nBeginOffset是起始偏移
	 * @param		nEndOffset是终止偏移，-1表示直接到文件的结束位置
	 * @return		对文件中的部分内容进行MD5计算的结果
	 */
	static string GetMD5( const string& strFilePath, ULONG nBeginOffset, INT nEndOffset = -1);
