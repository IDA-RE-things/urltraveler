...
string CMD5Checksum::GetMD5(const string& strFilePath)
{
	...
	::ReadFile(hFile,szBuffer, nBufferSize, &nRealRead, NULL);
	while( nRealRead > 0)
	{
		MD5Checksum.Update( (BYTE*)szBuffer, (ULONG)nRealRead );
		::ReadFile(hFile,szBuffer, nBufferSize, &nRealRead, NULL);
	}
...
//  [2/20/2009 ken]
string CMD5Checksum::GetMD5W(const wstring& strFilePath)
{
	//open the file as a binary file in readonly mode, denying write access 
...
string CMD5Checksum::GetMD5( const string& strFilePath, ULONG nBeginOffset, INT nEndOffset)
{
	//open the file as a binary file in readonly mode, denying write access 
	HANDLE hFile = ::CreateFileA( strFilePath.c_str(), 
								GENERIC_READ, 
	...
	else if( nEndOffset > 0)
		nEndOffset = (nEndOffset > (INT)nFileSize ? nFileSize : nEndOffset);
	else
		return "";
	...
	DWORD nLength = nEndOffset - nBeginOffset > 0 ? nEndOffset - nBeginOffset : 0 ;
	::SetFilePointer( hFile, nBeginOffset, NULL, FILE_BEGIN);

	char	szBuffer[1024];
	
	DWORD	nLeftForRead = nLength;
	DWORD	nRealRead = 0;

	while( nLeftForRead > 0)
	{
		if( nLeftForRead <= 1024)
		{
			::ReadFile(hFile,szBuffer, nLeftForRead, &nRealRead, NULL);
		}
		else
		{
			::ReadFile(hFile,szBuffer, 1024, &nRealRead, NULL);
		}

		MD5Checksum.Update( (BYTE*)szBuffer, (ULONG)nRealRead );

		nLeftForRead -= nRealRead;
	}
...
FUNCTION:		CMD5Checksum::GetMD5
DETAILS:		static, public
DESCRIPTION:	Gets the MD5 checksum for a std string
RETURNS:		string : the hexadecimal MD5 checksum for the specified data
ARGUMENTS:		strBuf  :	the string buf that should be calculated MD5
UINT nLength :	number of BYTEs of data to be checksumed
NOTES:			Provides an interface to the CMD5Checksum class. Any data that can
be cast to a BYTE array of known length can be checksummed by this
function. Typically, string and char arrays will be checksumed, 
...
*****************************************************************************************/
#if 0
string CMD5Checksum::GetMD5( string strBuf)
{
	CMD5Checksum MD5Checksum;
	MD5Checksum.Update( (BYTE*)strBuf.c_str(), (ULONG)strBuf.length() );
	return MD5Checksum.Final();
}
#endif
