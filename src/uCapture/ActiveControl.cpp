...
HINSTANCE CWindowWnd::GetInstance()
{
...
STDMETHODIMP CActiveXCtrl::OnInPlaceActivateEx(BOOL* pfNoRedraw, DWORD dwFlags)        
{
	...
	HWND hWndFrame = m_pOwner->m_hFrameWnd;
	HRESULT Hr = E_FAIL;
...
STDMETHODIMP CActiveXCtrl::GetWindowContext(IOleInPlaceFrame** ppFrame, IOleInPlaceUIWindow** ppDoc, LPRECT lprcPosRect, LPRECT lprcClipRect, LPOLEINPLACEFRAMEINFO lpFrameInfo)
{
	...
	*ppDoc = NULL;
	...
	lpFrameInfo->hwndFrame = m_pOwner->m_hFrameWnd;
...
HRESULT CActiveXCtrl::CreateActiveXWnd()
{
	...
	m_pOwner->m_hwndHost = m_pWindow->Init(this, m_pOwner->m_hFrameWnd);
	return S_OK;
}
...
void CActiveXWnd::DoVerb(LONG iVerb)
{
	...
	pUnk->DoVerb(iVerb, NULL, pOleClientSite, 0, m_hWnd, &m_pOwner->m_pOwner->m_rcItem);
}
...
bool CActiveXUI::DoCreateControl()
{
	...
	if( (dwMiscStatus & OLEMISC_INVISIBLEATRUNTIME) == 0 ) {
		Hr = m_pUnk->DoVerb(OLEIVERB_INPLACEACTIVATE, NULL, pOleClientSite, 0, m_hFrameWnd, &m_rcItem);
		::RedrawWindow(m_hFrameWnd, &m_rcItem, NULL, RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASE | RDW_INTERNALPAINT | RDW_FRAME);
	}
	IObjectWithSite* pSite = NULL;
	m_pUnk->QueryInterface(IID_IObjectWithSite, (LPVOID*) &pSite);
	if( pSite != NULL ) {
...
void CActiveXUI::SetMainFrame( HWND hWnd )
{
	m_hFrameWnd = hWnd;
}

void CActiveXUI::SetRect( CRect rc )
{
	m_rcItem = rc;
}
